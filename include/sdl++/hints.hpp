...
enum class hint {
    ...
    */
    ...
    /**
     *  \brief  A variable that causes SDL to use the old axis and button
    ...
    /**
    *  \brief  A string specifying SDL's threads stack size in bytes or "0" for
    ...
    * library (such as musl) which
    *  provides a relatively small default thread stack size (a few kilobytes
    * versus the default 8MB glibc uses).
    ...
    * source
    *  or those provided by the Chrome browser. In the later case, these
    * binaries require
    ...
    * is set to has
    *  SDL_WINDOW_OPENGL set (and running on WGL only, currently), then two
    * things will occur on the newly
    *  created SDL_Window:
    *
    *  1. Its pixel format will be set to the same pixel format as this
    * SDL_Window.  This is
    *  needed for example when sharing an OpenGL context across multiple
    * windows.
    *
    ...
    * created with SDL_CreateWindowFrom() should
    ...
    */
     ...
     * its
     *  users.  On Windows 8, 8.1, and RT, Microsoft mandates that this policy
     * be
     ...
     * policy
     *  link to the Settings charm.  This hint should only be set during app
     * init.
     *
     ...
     * 8.1, and RT,
     *  Microsoft mandates that this policy be available via the Windows
     * Settings charm.
     ...
     * A separate
     *  hint, SDL_HINT_WINRT_PRIVACY_POLICY_URL, is used to link to the actual
     * text of the
     *  policy.
     *
     ...
     * which
     *  it will check the contents of the hint,
     ...
     *  back button is pressed, or can set it in direct-response to a back
     * button
     *  being pressed.
     *
    ...
    /**
     *  \brief  A variable that dictates policy for fullscreen Spaces on Mac OS
     * X.
     *
     *  This hint only applies to Mac OS X.
     *
     *  The variable can be set to the following values:
     *    "0"       - Disable Spaces support (FULLSCREEN_DESKTOP won't use them
     * and
     *                SDL_WINDOW_RESIZABLE windows won't offer the "fullscreen"
     *                button on their titlebars).
     *    "1"       - Enable Spaces support (FULLSCREEN_DESKTOP will use them
     * and
     *                SDL_WINDOW_RESIZABLE windows will offer the "fullscreen"
     *                button on their titlebars).
     *
     *  The default value is "1". Spaces are disabled regardless of this hint if
     *   the OS isn't at least Mac OS X Lion (10.7). This hint must be set
     * before
    ...
    */
    ...
    /**
     * \brief Android APK expansion patch file version. Should be a string
     * number like "1", "2" etc.
     *
     * Must be set together with
    ...
    /**
     * \brief A variable to control whether certain IMEs should handle text
     * editing internally instead of sending SDL_TEXTEDITING events.
     *
     * The variable can be set to the following values:
     *   "0"       - SDL_TEXTEDITING events are sent, and it is the
     * application's
     *               responsibility to render the text from these events and
     *               differentiate it somehow from committed text. (default)
     *   "1"       - If supported by the IME then SDL_TEXTEDITING events are not
     * sent,
     *               and text that is being composed will be rendered in its own
     * UI.
     */
    ime_internal_editing,

    /**
    * \brief A variable to control whether mouse and touch events are to be
    ...
    *   "1"       - Mouse events will be handled separately from pure touch
    * events.
    *
...
template <typename Func>
[[gnu::warn_unused_result]] auto make_hint_callback(hint name, Func&& func) {
